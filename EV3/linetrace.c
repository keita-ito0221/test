...
//   (float)        : gyro sensor data
//   (float)_offset : gyro sensor offset data
...
/* Model step function */
void linetrace_control(float args_cmd_forward, float args_cmd_turn, float
...
{
  {
     ...
     */
    tmp_theta = ((DEG2RAD * args_theta_m_l) + (DEG2RAD *
      args_theta_m_r)) * 0.5F;
     ...
     */
  	float K_F[2] = {-0.86526F, -1.14828F*0.70F};
    tmp[0] = ud_theta_ref;
    tmp[1] = tmp_thetadot_cmd_lpf;
    tmp_theta_0[0] = tmp_theta;
    tmp_theta_0[1] = (tmp_theta_lpf - ud_theta_lpf) / EXEC_PERIOD;
